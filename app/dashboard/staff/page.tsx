...
export default function StaffDashboard() {
  ...
  const loadAllBookings = async () => {
    try {
      ...
      if (bookingsData && bookingsData.length > 0) {
        ...
        // Fetch hotels directly from database with privileged access via API
        let allHotels = []
        try {
          const hotelsResponse = await fetch('/api/admin/bookings-data')
          if (hotelsResponse.ok) {
            const responseData = await hotelsResponse.json()
            console.log('API Response:', responseData)
            allHotels = responseData.hotels || []
            console.log('Hotels from API:', allHotels.length, allHotels)
          } else {
            console.error('Hotels API response not ok:', hotelsResponse.status)
          }
        } catch (apiErr) {
          console.error('Failed to fetch hotels from API:', apiErr)
        }

        // If no hotels from API, try direct Supabase fetch
        if (allHotels.length === 0 && hotelIds.length > 0) {
          console.log('Trying direct Supabase fetch for hotels:', hotelIds)
          const { data: directHotels } = await supabase
            .from('hotels')
            .select('id, name, location')
            .in('id', hotelIds)
          allHotels = directHotels || []
          console.log('Hotels from direct Supabase:', allHotels.length, allHotels)
        }
  ...
  const updateBookingStatus = async (bookingId: string, newStatus: string) => {
    try {
      ...
      // Refresh bookings data and payment stats
      await Promise.all([loadAllBookings(), loadPaymentStats()])
    } catch (err) {
