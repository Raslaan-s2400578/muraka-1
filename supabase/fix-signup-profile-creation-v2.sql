-- Fix signup profile creation issue - Version 2
-- Run this in Supabase SQL Editor
-- This version uses DO blocks to avoid duplicate policy errors
...
-- Step 4: Fix RLS policies using DO blocks to avoid duplicates
DO $$
BEGIN
  ...
  EXECUTE 'CREATE POLICY profiles_select_admin ON profiles FOR SELECT USING (
    EXISTS (
  ...
  EXECUTE 'CREATE POLICY profiles_update_admin ON profiles FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
      AND profiles.role = ''admin''
